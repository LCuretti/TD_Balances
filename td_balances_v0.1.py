...
class Position:
    ...
    def __init__(self, symbol):
        self.symbol = symbol
        self.quantity = 0.0
        self.fifo = {'holdings': [], 'allocated': 0.00}
        self.lifo = {'holdings': [], 'allocated': 0.00}
        self.orders = []
        self.transactions = []
...
class Balances:
    ...
    def _update_position_balances(self, transaction):
        ...
        allocated_balance_update = -transaction['netAmount'] + results['fifo']['gain']
        self.fifo['allocated'] += allocated_balance_update
        self.lifo['allocated'] += allocated_balance_update
    ...
    def transaction_interpretation_ameritrade(self, transaction):
        '''
...
def get_historical_transactions(td_api, historical_transactions_json_file):
    '''
    ...
    transactions = td_api.get_transactions(transaction_type = 'ALL')
    overlap = False
    ...
    if not overlap:
        load_transactions_until_overlap(td_api, transactions)
...
def load_transactions_until_overlap(td_api, transactions):
    '''
    ...
    while True:
        if transactions:
            ...
            if end_date[-2:] in ('30','31','01'): #to avoid midnait interest that may get in both fetchs
            ...
            fetched_transactions = td_api.get_transactions(transaction_type = 'ALL',
...
if __name__ == '__main__':
    ...
    tdapi = TDApi(user_data)

    account = tdapi.get_accounts(fields = ['orders','positions'])['securitiesAccount']
    # Uso del sistema
    balances_system = Balances()
    allTransactions = get_historical_transactions(tdapi, JSON_FILE)
    #allTransactions = read_json_file(JSON_FILE)

    for t in allTransactions:
        balances_system.transaction_interpretation_ameritrade(t)
